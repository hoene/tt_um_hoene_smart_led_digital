...
* TEST_MODE (i,+9) True if the test_mode selected.
...
* OUT (o,+6) Low-pass filtered data signal output
...
### Inputs and Outputs
* IN (i,+6) Data signal input from low pass filters's OUT
* OUT_DATA (o,+7) data signal bit. It is only valid while OUT_CLK is high, too.
* OUT_CLK (o,+7) one cycle high indicating a new OUT_DATA
* OUT_ERROR (o,+7) the manchester decoding is not according to definition. It remain in error state till a good long bit is detected (e.g., a one and zero bit sequences)
...
### Inputs and Outputs
* IN_DATA (i,+7) the bit from the Manchester decoding
* IN_CLK (i,+7) the clock signal from the Manchester decoding
* IN_ERROR (i,+7) The manchester decoding is in error state
* OUT_FRAME (o,+8) True if the frame has started

## 5. Counting Bits and LEDs
...
* IN_DATA (i,+7) the bit from the Manchester decoding
* IN_CLK (i,+7) the clock signal from the Manchester decoding
* IN_FRAME (i,+8) if true, the data is in a frame
* BIT_COUNTER (o,+9) counting the 32 bits
* TEST_MODE (o,+9) 4095 LED data wordss switches the test mode on.

## 6. Protocol Unit. 
The protocol units selects the right and correct data for the LED PWM. Based on the input modes, this modules selects the right LED data word which shall to be forwarded to the LED PWM.
...
* BIT_COUNTER (i+9) counting the 32 bits
* PWM_SET (o+8) This data word shall be selected.
* OUT_DATA (o+8) The outgoing data signal, modified if required.
* OUT_CLK (o+8) Clock for the outgoing decoder
* OUT_CLK_LED (o+8) Clock for the bit swift register
...
### Inputs and Output
* IN_DATA (i+8) Valid data within a frame from selector
* IN_CLK (i+8) Valid clock within a frame from selector
* PWM_SET (i+8) If true, store data in output register
* OUTPUT_DATA (o+9) 30 bit output register

## 9. Pulse Width Modulator
Converts input bits to PWM outputs.

### Inputs and Output
* DATA_RED[9:0] (i+9) Brightness of red LED
* DATA_GREEN[9:0] (i+9) Brightness of green LED
* DATA_BLUE[9:0] (i+9) Brightness of blue LED
* OUT_RED (o+1024) LED red on
* OUT_GREEN (o+1024) LED green on
* OUT_BLUE (o+1024) LED blue on

## 10. Manchester Encoder
Converts the incoming data stream into a modified outgoing data stream by inverting the signal sometimes.

### Inputs and Output
* SWAP (i,+8) Swap bit information
* IN (i,+6) low pass filtered signal
* DOUT (o+9) The Manchester output
