...
module tt_um_hoene_manchester_decoder #(
  ...
  always @(posedge clk) begin
    ...
    if (!rst_n) begin
      ...
      middle <= 0;
      out_pulsewidth <= BIT_LENGTH_DIV4 * 4;
    end else if (counter == BIT_LENGTH_DIV4 * 8) begin
      counter <= 0;
      out_data <= 0;
      out_clk <= 0;
      out_error <= 1;
      ...
      if (counter >= BIT_LENGTH_DIV4 * 3) begin
        ...
        counter <= 0;
      end else if (counter >= BIT_LENGTH_DIV4 && !out_error) begin
        if (!middle) begin
