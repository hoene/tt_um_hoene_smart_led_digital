...
async def test_protocol_bin(dut):
    ...
    for parity in range(0, 2):
        ...
        dut.protocol_in0selected.value = 0
        ...
        assert dut.protocol_pwm_set.value == 0
        assert dut.protocol_error.value == 0
        assert dut.protocol_state.value == 2
        ...
        # insync is one, bits is 31, data is one -> second LED data
        dut.protocol_in_data.value = parity  # 0 is correct
        dut.protocol_bits.value = 31
        await ClockCycles(dut.clk, 1)
        dut.protocol_in_clk.value = 1
        await ClockCycles(dut.clk, 1)
        ...
        # check output signals and check parity
        assert dut.protocol_out_led_clk.value == 0
        assert dut.protocol_out_clk.value == 1
